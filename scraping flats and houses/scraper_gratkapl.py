...
class PageScrapper:
    ...
    def parse_advertisement(self, url):
        ...
        data['cena'] = float(container.find('div', {'class': 'priceInfo'}).find('span',{'class':"priceInfo__value"}).get_text().strip().replace('zł', '').replace(' ', '').replace(',','.'))
        data['cena_waluta'] = container.find('div', {'class': 'priceInfo'}).find('span',{'class':"priceInfo__currency"}).get_text().strip()
        
        if container.find('div', {'class': 'priceInfo'}).find('span',{'class':"priceInfo__additional"}) is not None:
...
URL1 = 'https://gratka.pl/nieruchomosci/mieszkania/gdansk/sprzedaz?page=1&cena-calkowita:min=100000'
URL2 = 'https://gratka.pl/nieruchomosci/mieszkania/gdynia/sprzedaz?page=1&cena-calkowita:min=100000'
